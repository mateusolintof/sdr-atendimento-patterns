...
const presenceComposing = node({
  ...
  config: {
    ...
    parameters: {
      method: 'POST',
      url: expr("={{ $env.EVOLUTION_BASE_URL }}/chat/sendPresence/{{ $env.EVOLUTION_INSTANCE_NAME }}"),
...
const sendEvolutionText = node({
  ...
  config: {
    ...
    parameters: {
      method: 'POST',
      url: expr("={{ $env.EVOLUTION_BASE_URL }}/message/sendText/{{ $env.EVOLUTION_INSTANCE_NAME }}"),
      authentication: 'genericCredentialType',
      genericAuthType: 'httpHeaderAuth',
      sendHeaders: true,
      specifyHeaders: 'keypair',
